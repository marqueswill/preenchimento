...
def carregar_planilha():
    ...
    caminho_nl_diaria = f"C:\\Users\\{username}\\OneDrive - Tribunal de Contas do Distrito Federal\\General - SECON\\FLUXO_DIÁRIAS\\TABELA_NL_DIARIAS_1.xlsx"
...
def preencher_cabecalho(driver, dados):
...
def preencher_nl(driver, dados, aba):
...
def nova_aba(driver):
    ...
    driver.get(f"https://siggo.fazenda.df.gov.br/{ANO_ATUAL}/afc/nota-de-lancamento")
    esperar_carregamento(driver)
    time.sleep(2)  # tempo para carregar nova aba
...
def main():
    ...
    driver = setup_driver()
    abas = int(ceil(dataframe.shape[0] / 24))

    # login_siggo(driver)
    esperar_login(driver)

    for aba in range(abas):
        nova_aba(driver)
        preencher_cabecalho(driver, dataframe)
        preencher_nl(driver, dataframe, aba)
